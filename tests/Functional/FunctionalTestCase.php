    ...
    protected function publishMalformedAmqpMessage(string $queue, array $options = []): void
    {
        ...
        if (!in_array('missingMessageId', $options, true)) {
            if (in_array('invalidUuid', $options, true)) {
                $headers['X-Message-Stamp-Freyr\MessageBroker\Stamp\MessageIdStamp'] = json_encode([['messageId' => 'not-a-uuid']]);
            } else {
                $headers['X-Message-Stamp-Freyr\MessageBroker\Stamp\MessageIdStamp'] = json_encode([['messageId' => '01234567-89ab-cdef-0123-456789abcdef']]);
            }
        }
