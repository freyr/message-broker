...
final class Configuration implements ConfigurationInterface
{
    public function getConfigTreeBuilder(): TreeBuilder
    {
        ...
        $treeBuilder->getRootNode()
            ...
            ->cannotBeEmpty()
            ->validate()
            ->ifTrue(fn (mixed $v): bool => !is_string($v) || preg_match('/^[a-zA-Z_][a-zA-Z0-9_]*$/', $v) !== 1)
            ->thenInvalid('Table name must contain only alphanumeric characters and underscores. Got: %s')
            ->end()
            ->end()
            ->end()
            ->end()
