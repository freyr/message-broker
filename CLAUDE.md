...
│   │   │   └── AmqpExchange.php            # Attribute for custom exchange
...
services:
  ...
  # Reads existing MessageIdStamp from envelope and publishes via resolved SenderInterface
...
```php
// ✅ Bridge middleware reads existing stamp and publishes to AMQP via sender locator
final readonly class OutboxToAmqpBridge implements MiddlewareInterface
{
    ...
    public function handle(Envelope $envelope, StackInterface $stack): Envelope
    {
        ...
        // Resolve sender by name and publish to AMQP (no nested bus dispatch)
